...
SELECT LEFT(SALES_DATE, 4) AS YEAR, SUBSTRING(SALES_DATE, 6, 2) AS MONTH, COUNT(DISTINCT USER.USER_ID) AS PURCHASED_USERS, ROUND(COUNT(DISTINCT USER.USER_ID)/(
SELECT COUNT(DISTINCT USER_ID) FROM USER_INFO
    WHERE JOINED LIKE '%2021%'
), 1) AS PUCHASED_RATIO FROM USER_INFO AS USER
INNER JOIN ONLINE_SALE AS SALE ON USER.USER_ID = SALE.USER_ID
WHERE USER.JOINED LIKE '%2021%'
GROUP BY SUBSTRING(SALES_DATE, 6, 2)
ORDER BY SALES_DATE
